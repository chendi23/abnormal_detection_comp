...
class DataParser:
    ...
    def parse(self, df=None):
        assert not (df is None)
        if gl.TARGET_COL_NAME in df.columns:
            labels = df[gl.TARGET_COL_NAME].tolist()
        else:
            labels = []
        ...
        for i in dfi.columns:
            if i in self.feature_dict_ob.ignore_cols:
                ...
                dfi[i] = dfi[i].fillna(
                    int(np.random.choice([min(self.feature_dict[i].values()), max(self.feature_dict[i].values())], 1)),
                    inplace=False)
                dfv[i] = 1
