...
class DeepFM(BaseEstimator, TransformerMixin):
    def __init__(self, feature_size, field_size, model_dir,
                 ...
                 verbose=False, random_seed=2016, num_checkpoints=6,
                 use_fm=True, use_deep=True,
                 loss_type="logloss", eval_metric=roc_auc_score,
                 l2_reg=0.0, greater_is_better=True, checkpoint_every=5,
                 trainIndex=0
                 ):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():
            ...
            if self.optimizer_type == "adam":
                train_op = tf.train.AdamOptimizer(learning_rate=self.learning_rate, beta1=0.9, beta2=0.999,
                                                  epsilon=1e-8)
                train_step = train_op.minimize(self.loss)

            elif self.optimizer_type == "adagrad":
                train_op = tf.train.AdagradOptimizer(learning_rate=self.learning_rate,
                ...
                train_op = tf.train.MomentumOptimizer(learning_rate=self.learning_rate, momentum=0.95).minimize(
                    self.loss)
            elif self.optimizer_type == "yellowfin":
                train_op = YFOptimizer(learning_rate=self.learning_rate, momentum=0.0)
                train_step = train_op.minimize(self.loss)
    ...
    def training_termination(self, valid_result):
        if len(valid_result) > 5:
            if self.greater_is_better:
                if valid_result[-1] < valid_result[-2] < valid_result[-3] < valid_result[-4] and valid_result[-4] < \
                        valid_result[-5]:
                    return True
            else:
                if valid_result[-1] > valid_result[-2] > valid_result[-3] > valid_result[-4] and valid_result[-4] > \
...
deepfm = DeepFM(feature_size=50, field_size=2)

def gen_test_data():
    for i in range(50):
        Xi_ = []
        Xv_ = []
        pick_Xi= np.random.choice(np.arange(100), 20, replace=False)
        pick_Xv = np.random.randint(0, 2, 20)
        sorted_pick = np.sort(pick_Xi)
        Xi_.append(sorted_pick)
        Xv_.append(pick_Xv)
