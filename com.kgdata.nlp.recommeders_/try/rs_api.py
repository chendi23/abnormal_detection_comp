...
from recommend_scenario.hot_recommend import HotRecommend
...
class GetHotRecommendList(Resource):
    ...
    def post(self):
        try:
            ...
            hot_rs_list = hot_rs.get_recommend_list(model_version_id=model_version_id)
            return {
                'retCode': 0,
                'data': {
...
class RecallModelTrain(Resource):
    ...
    def post(self):
        try:
            ...
            model_version_id = api.payload["modelVersionId"]
            params_dict = api.payload["paramsDict"]
...
class RSUpdatePersonas(threading.Thread):
    ...
    def run(self):
        logger.debug('开始线程：' + self.name)
        self.dao.update_related_items_by_users(self.user_id_list)
        logger.debug('退出线程：' + self.name)
